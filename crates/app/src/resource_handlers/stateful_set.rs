...
pub(crate) fn watcher(context: TypedWatcherContext) -> Option<Box<dyn ResourceWatcher>> {
    matches_namespaced_resource::<StatefulSet>(&context)
        .then(|| namespaced_typed_watcher::<StatefulSet>(context, extract))
}

pub(crate) fn table_definition(api_resource: &ApiResource) -> Option<ResourceTableDefinition> {
    matches_namespaced_api_resource::<StatefulSet>(api_resource).then(|| ResourceTableDefinition {
        columns: vec![column(READY_COLUMN, "Ready", 90.0)],
    })
}

fn extract(resource: &StatefulSet) -> MinimalResource {
