...
pub(crate) fn watcher(context: TypedWatcherContext) -> Option<Box<dyn ResourceWatcher>> {
    matches_namespaced_resource::<ReplicationController>(&context)
        .then(|| namespaced_typed_watcher::<ReplicationController>(context, extract))
}

pub(crate) fn table_definition(api_resource: &ApiResource) -> Option<ResourceTableDefinition> {
    matches_namespaced_api_resource::<ReplicationController>(api_resource).then(|| {
...
fn extract(resource: &ReplicationController) -> MinimalResource {
